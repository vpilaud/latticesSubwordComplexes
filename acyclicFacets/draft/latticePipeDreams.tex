...
\begin{enumerate}
\item While the sets~$\linearExtensions(I)$ are order convex, they may not be intervals. See \cref{fig:counterExample2}. Another example with~$\omega = \omega_\circ$ can be found in~\cite[Figure~9]{PilaudStump-brickPolytope}. 
%
\begin{figure}[p]
	\centerline{\includegraphics[scale=.8]{counterExample2}}
	\caption{The sets~$\linearExtensions(I)$ (blue bulles) are not always intervals.}
	\label{fig:counterExample2}
\end{figure}

\item The lower set~$\linearExtensions(Q,\omega)$ may have more than one maximal element, and~$\omega$ is not necessarily maximal in~$\linearExtensions(Q,\omega)$. See \cref{fig:counterExample1}.

\item Some of the sets~$\linearExtensions(I)$ may not be included in, nor even meet~$[e,\omega]$. See \cref{fig:counterExample1}.
%
%\begin{figure}[h]
...
%
\begin{figure}[p]
	\centerline{\includegraphics[scale=.8]{counterExample1}}
...
\end{figure}
\end{enumerate}
Note that \cref{fig:counterExample1,fig:counterExample2} represent type~$A$ subword complexes.
...
For the purpose of this proof, let us write~$\leqslant_B$ the strong Bruhat order, defined by~$x \leqslant_B y$ if and only if a reduced expression of~$y$ has a reduced expression of~$x$ as a subword, and~$x \prec_B y$ the covers of this order. We note that for any word~$Q$ and any~$x\in W$, we can find a reduced expression of~$x$ as a subword of~$Q$ if and only if~$x \leqslant_B Dem(Q)$ the demazure product of~$Q$.
...
Let~$\subwordComplex(Q,w)$ be a non-empty subword complex. Then for any facet~$I \in \subwordComplex(Q,w)$ and for any simple root~$\alpha$ such that~$w \prec_B s_\alpha w \leqslant_B Dem(Q)$, we have~$\alpha \in \Roots(I)$.
\end{proposition}
...
Let us call~$Q'$ the suffix of~$Q$ starting at index~$j+1$, and~$I'_1,I'_2$ the restrictions of~$I_1$ and~$I_2$ to~$Q'$. The subwords induced by~$I'_1$ and~$I'_2$ on~$Q'$ are suffixes of reduced subwords of~$Q$, so they are also reduced. Moreover, since~$j \in I_1$, we know that~$I'_1$ is a facet of~$\subwordComplex(Q',v)$; this means that this subword complex is not empty and that~$v \leqslant_B Dem(Q')$. Similarly, since~$j \notin I_2$, we know that~$I'_2$ is a facet of~$\subwordComplex(Q',sv)$. By combining the previous statements, we know that~$sv \prec_B v \leqslant_B Dem(Q')$ (or equivalently and with~$v' = sv$, that~$v' \prec_B sv' \leqslant_B Dem(Q')$). We can thus apply \cref{prop:JSDemazureCone} to obtain that~$\alpha \in \Roots(I'_2)$.
