...
%%For instance, for the reversed permutation~$\omega = n \dots 2 1$, the permutation~$0\omega$ is the permutation~$\rho_n$ considered in \cref{exm:Tamari1}.
%%The permutations $\omega_\nu$ associated to lattice paths $\nu$ are those of the form~$1\oplus w$ for some dominant permutation $w$, see~\cite{CeballosPadrolSarmiento}. 
%
%\begin{openproblem}%[The acyclic property for $\nu$-Tamari lattices]
%\label{prob:nuAcyclicProperty}
%Show that if $\omega$ is a dominant permutation then all pipe dreams with permutation $0 \omega$ are acyclic. %In particular, the corresponding $\nu$-Tamari lattice is the increasing flip poset on acyclic pipe dreams~$\acyclicPipeDreams(1\oplus \omega)$.
%\end{openproblem}
%
...
We are grateful to Florent Hivert for various discussions. We also thank Martin Rubey for his comments which led us to think about Open Problem~\ref{prob:nuAcyclicProperty}. 
