...
We can now apply \cite[proposition 3.14]{JahnStump} to~$I'_2$ the restriction of~$I_2$ to~$Q'$, which tells us that~$\mathcal{E}^+(sv, Dem(Q)) \subseteq \cone \Roots(I'_2)$. Since~$sv \less_B v \leqslant Dem(Q)$, then~$\alpha \in \mathcal{E}^+(sv, Dem(Q))$ and so~$\alpha \in \cone \Roots(I'_2)$.
