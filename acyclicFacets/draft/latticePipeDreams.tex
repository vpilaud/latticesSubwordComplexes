...
More precisely, at step~$t$, we insert a pipe starting at row~$\pi(t)$, ending at column~$\omega^{-1}(\pi(t))$, and whose northeast elbows are precisely covering the free southeast elbows in the rectangle~$[\pi(t)] \times [\omega^{-1}(\pi(t))]$.
...
At each step~$t$ of the insertion algorithm, the number of free southeast elbows in the rectangle~$[\pi(t)] \times [\omega^{-1}(\pi(t))]$ is
\[
