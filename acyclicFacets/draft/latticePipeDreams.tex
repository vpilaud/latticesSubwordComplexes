...
The main example is when $\omega=n\dots21$ is the reverse permutation, see \cref{exm:Tamari1}.
In this case, the increasing flip poset on~$\pipeDreams(0 \omega)$ is the classical Tamari lattice. Removing vertex $0$ from the contact graph of a pipe dream~$ P\in \pipeDreams(0 \omega)$ returns the binary tree corresponding to $P$ (with the leaves removed and the internal nodes labeled in inorder from $1$ to $n$).
The edges of the binary tree are oriented going away from the root of the tree.
See \cref{fig:bijection}.

Given a pipe dream $P$, we write $i\to j$ whenever $i$ has an elbow weakly northwest of $j$. \cref{lem:rectangle} gives us that $i\to j$ implies $i\contactLess{P} j$. 
When $P$ is not acyclic, we can find a cycle ${\bf C}=(i_1, i_2, \ldots, i_m)$ of the contact graph, where $i_1 \to i_2\to\cdots\to i_m\to i_1$. 
We say that $\bf{C}$ is a \defn{reduced cycle} if it is not possible to remove any pipe from $\bf C$ and still have a cycle. 
Given a reduced cycle $\bf C$, we can also assume that $i_1 = \min\{i_1, \dots, i_m\}$.
\vincent{Maybe I would put this cycle story inside the proof.}

To describe the location of crossings and contacts (elbows), we use the coordinate $(c,r)$, where~$r$ is the row and $c$ is the column (ranging from $0$ to~$n$).
...
\begin{proof}
Let $(c,r)$ denote the position of the crossing~$x$. Since $i$ has an elbow southwest of $x$, we have $r\leq i-1$.
...
\begin{proof}
For the forward implication, assume that $\omega$ is not dominant.
Let $\greedyPipeDream$ be the greedy pipe dream of $0\omega$. 
\vincent{I believe you wanted the antigreedy here?}
It has no crossing in row 0, and cannot be a partition starting at row 1.
\vincent{I don't like the way it is written here. I would say explicitly the connection between the crossings of the greedy pipe dream and the Rothe diagram of~$\omega$.}
There must be a row $r>1$ with more crossing than in row $r-1$.
\vincent{You mean really inclusionwise on columns?}
...
Let $Q$ be the pipe dream obtained from $\greedyPipeDream$ by the flip of the crossing at $(c,r)$ with the contact at $(c+1,r-1)$.
The contact of $Q$ at $(c,r)$ gives $r\contactLess{Q} k$.
In row $0$ we only have contacts, we have $\omega^{-1}(k)<\omega^{-1}(r)$ and \cref{lem:rectangle} gives that $k \contactLess{Q} r$, hence $Q$ is not acyclic.

For the converse, assume we have a pipe dream $P$ that is not acyclic. The contact graph must contain a reduced cycle ${\bf C}=(i, j_1, \ldots, j_m)$ with $i<j_k$ for all $k \in [m]$. 
If $m=1$, then $i$ and $j_1$ must cross , $i\to j_1$ implies there is an elbow of $i$ southwest of the crossing and $j_1\to i$ implies there is an elbow of $j$ northeast of the crossing. \cref{lem:notDominant} implies $\omega$ is not dominant. We consider the case where $m>1$.
For $i\to j_1$, there is an elbow of~$i$ at $(a,b)$ weakly northwest of an elbow of~$j_1$ at $(c_1,d_1)$ an elbow.
Similarly, for $j_m\to i$, there is an elbow of~$j_m$ at $(a_m,b_m)$ weakly northwest of an elbow of~$i$ at~$(c,d)$.
Since the cycle $C$ is reduced, we must have that $(a_m,b_m)$ is not northwest $(c_1,d_1)$. That is $c_1<a_m$ or $d_1<b_m$.

For $d_1<b_m$, we must have $a_m<c_1$ since the pipe $i$ passes northwest of $(c_1,d_1)$ and southeast of $(a_m,d_m)$.
The pipe $j_1$ cannot have an elbow southeast of $(a_m,b_m)$, as otherwise $j_m\to j_1$ would contradict the fact that $\bf C$ is reduced.
The pipes $i<j_1$ must cross in some column $<a_m$ since $j_1$ must remain strictly west of $a_m$ in rows $>b_m$, but the pipe $i$ must have and elbow southeast of $(a_m,b_m)$.
Now, in rows $\le b_m$ the pipe $j_1$ must remain northwest of the pipe $i$. 
It is them impossible to have an elbow in position $(c_1,d_1)$ southeast of $i$ in a row $d_1<b_1$. This is a contradiction and it excludes this case.

For $c_1<a_m$, similarly to above,  we must now have $b_m<d_1$ and the pipe $j_m$ cannot have an elbow northeast of $(c_1,d_1)$.
The pipes $i<j_m$ must cross in some column strictly between $c_1$ and $a_m$, since the pipe $j_m$ must remain strictly south of $d_1$ in columns $<c_1$, but the pipe $i$ must have an elbow northwest of $(c_1,d_1)$ and the pipe $j_m$ has an elbow at $(a_m,b_m)$ northwest of $i$.
For the crossing of $i$ and $j_m$,
the pipe $i$ has an elbow at $(a,b)$ southeast of the crossing since $a\le c_1$ and the pipe $j_m$ has an elbow at $(a_m,b_m)$ northeast of the crossing.
\cref{lem:notDominant} implies that $\omega$ is not dominant and conclude the proof.
\end{proof}
...
We are grateful to Florent Hivert for various discussions. We also thank Martin Rubey for his comments which led us to think about Theorem~\ref{prob:nuAcyclicProperty} and
we are very grateful to Lucas Gagnon who shared with us an independent proof of the converse direction of Theorem~\ref{prob:nuAcyclicProperty}, this allowed us to simplify grately our argument.
