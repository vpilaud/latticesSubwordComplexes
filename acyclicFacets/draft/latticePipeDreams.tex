...
\newcommand{\subwordAcyclicFacets}{\mathcal{F}^\mathrm{ac}} % subword complex
...
A pipe dream is an arrangement of pipes in the triangular shape, each entering along the vertical side and exiting along the horizontal side.
...
The pipes in a pipe dream have some crossings and some contacts, and the contact graph records all contacts of a pipe dream.
A flip in a pipe dream exchanges a contact between with a crossing between two pipes, and the flip is increasing when the contact is southeast of the crossing involved in the flip.
The acyclic pipe dreams are the pipe dreams with an acyclic contact graph, and they naturally appear in the study of brick polytopes~\cite{PilaudSantos-brickPolytope}.
...
\item the Hasse diagram of the corresponding lattice quotient is the increasing flip graph on acyclic pipe dreams with exiting permutation~$\omega$ (\cref{subsec:pipeDreamQuotient}).
\end{itemize}
In summary, we obtain the following statement.

\begin{theorem*}
For any permutation~$\omega$, the increasing flip graph on acyclic pipe dreams with exiting permutation~$\omega$ is the Hasse diagram of the lattice quotient of the interval~$[e,\omega]$ of the weak order by the pipe dream congruence of~$\omega$.
\end{theorem*}
...
We then explore in \cref{sec:furtherTopics} some natural further topics.
...
Finally, we discuss in \cref{sec:subwordComplexes} a conjectural extension of our results to subword complexes in finite Coxeter groups~\cite{KnutsonMiller-subwordComplex}.


\begin{conjecture}
For any sorting and alternating word~$Q$ and any~$\omega \in W$, the 



the subword complex congruence~$\equiv_{Q,\omega}$ is a lattice congruence of the interval~$[e,\omega]$ of the weak order.
\end{conjecture}
...
We then define linear extensions of facets of subword complexes (\cref{subsec:linearExtensionsFacets}) and present two theorems (\cref{subsec:twoTheorems}) and two conjectures (\cref{subsec:twoConjectures}) about them.
...
Two facets $I,J \in \subwordFacets(Q,\omega)$ are connected by a \defn{flip} if they differ one element: $I \ssm \{i\} = J \ssm \{j\}$ for some $i\in I$ and $j\in J$ with $i \ne j$.
...
\begin{example}
\vincent{todo}
Connection root configuration --- contact graph.
\end{example}
...
\vincent{Is linear extension a good term? Should we say $W$-extension?}.
...
Connection linear extensions of contact graphs.
\end{example}
...
\[
\pi(\Phi^+) \smallsetminus \{-\beta\} = \pi'(\Phi^+) \smallsetminus \{\beta\} = \pi(\Phi^+) \cap \pi'(\Phi^+).
\]
...
\end{array}    
\end{equation}
By  \cref{lem:rootFunctionFlips}~\eqref{lem:rootFunctionFlips3}, we obtain that flipping $j$ to $i$ creates the new facet $I' = I \smallsetminus \{j\} \cap \{i\}$ whose corresponding restricted list looks like
\begin{equation}
\begin{array}{ccc}
  \beta &, \beta, \dots, &\beta.\\
      i && j
...
Since $\linearExtensions(I)\neq \varnothing$ if and only if $I$ is acyclic by \cref{lem:linearExtensionsAcyclic}, we have the following straight forward corollary.

\begin{corollary}\label{coro:linearExtensionsPartitionSubwordComplexes}
...
If $\pi \in \linearExtensions(I)$ for some facet~$I$ of~$\subwordComplex(Q, \omega)$, then by \cref{prop_jahnStump_intersection} we have 
\[
...
\subsection{Two conjectures on linear extensions of facets}
\label{subsec:twoConjectures}

In this section, we present two conjectural generalizations of the results of \cref{subsec:pipeDreamCongruence,subsec:pipeDreamQuotient}.
We first introduce the analogue of \cref{def:pipeDreamCongruence} for subword complexes.

\begin{definition}
\label{def:pipeDreamCongruenceSubwordComplexes}
For a non-empty subword complex $\subwordComplex(Q,\omega)$, the \defn{subword complex congruence} is the equivalence relation~$\equiv_{Q,\omega}$ on the set~$\linearExtensions(Q,\omega) \eqdef \sqcup_{I \in \subwordAcyclicFacets(Q, \omega)} \linearExtensions(I)$  whose equivalence classes are the sets~$\linearExtensions(I)$ of linear extensions of the acyclic facets~$I$ of~$\subwordComplex(Q, \omega)$.
In other words, $\pi \equiv_{Q,\omega} \pi'$ if and only if $\pi$ and $\pi'$ are linear extensions of the same facet.  
\end{definition}

Note that the subword complex congruence indeed defines an equivalence relation by \cref{coro:linearExtensionsPartitionSubwordComplexes}.
However, it is in general not a lattice congruence for two obvious reasons:
\begin{itemize}
\item while~$\linearExtensions(Q,\omega)$ is a lower set of the weak order containing~$[e, \omega]$ by \cref{thm:linearExtensionsPartitionSubwordComplexA}, it does not necessarily have a maximal element, hence it is not necessarily a lattice,
...
\end{itemize}
These two issues can however be bypassed with additional assumptions on the word~$Q$.
Recall that a word~$Q$ is
\begin{itemize}
\item \defn{sorting} if it contains a reduced expression of $\wo$ (see just before \cref{thm:linearExtensionsPartitionSubwordComplexB}),
\item \defn{alternating} if all non-commuting pairs $s, t\in S$ alternate within $Q$ (see \cite{PilaudSantos-brickPolytope, CeballosLabbeStump}).
\end{itemize}
By \cref{thm:linearExtensionsPartitionSubwordComplexB}, $\linearExtensions(Q,\omega) = [e, \omega]$ if $Q$ is sorting.

\begin{conjecture}
If the word~$Q$ is sorting and alternating, then for any~$\omega \in W$, the subword complex congruence~$\equiv_{Q,\omega}$ is a lattice congruence of the interval~$[e,\omega]$ of the weak order.
\end{conjecture}

This conjecture holds in type~$A_n$: our specific proof of~\cref{thm:pipeDreamCongruence} can be extended to arbitrary sorting and alternating words in type~$A_n$ as shown in~\cite{Cartier}.
It is also supported by extensive computer experiments: we verified it in types...
\cesarm{Noemie: in which cases did you check this conjecture? Type $A_n$ for n=???, B,C,D,....?} 

\vincent{I am working below this line}



\cesarm{To do: meet to discuss about this section? put it in final form...}
...
In analogy to the case of pipe dreams in \cref{subsec:proofCongruence}, one might expect that the quotient~$[e,\omega]/\equiv_{Q,\omega}$ is isomorphic to the restriction of the increasing flip poset of $\subwordComplex(Q,\omega)$ to the set of acyclic facets.
...
Let $Q=(s_1,s_2,s_1,s_2,s_1,s_2)$ and $\omega=\wo=s_1s_2s_1=s_2s_1s_2$.
The subword complex $\subwordComplex(Q,\omega)$ has eight facets, six of which are acyclic.
    All the fibers of $\equiv_{Q,\omega}$ are singletons and the lattice quotient~$[e,\omega]/\equiv_{Q,\omega}$ coincides with the weak order of type $A_2$.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item If $Q_{[m]\smallsetminus I^{j-1}\smallsetminus \{j\}}$ contains a reduced expression of $\omega$ with prefix $Q^{j-1}$ then let 
        \[
        I^j=I^{j-1}\cup \{j\}.
        \]        
...
\begin{proof}
    ...
    {\bf Claim}: $Q^j$ is a reduced expression which is the prefix of a reduced expression of $\omega$ in $Q_{[m]\smallsetminus I^j}$.

    \smallskip
    We prove the claim by induction on $j$.

    For $j=0$, $Q^0$ is the empty word which is reduced by definition.
    Furthermore, $Q_{[m]\smallsetminus I^0 }=Q$ contains a reduced expression of $\omega$ because the subword complex is non-empty.
The empty word is a prefix of this reduced expression. 
  ...
  \end{cases}
    ...
    Moreover, it is a prefix of a reduced expression of $\omega$ in $Q_{[m]\smallsetminus I^{j-1}}$.
    Since $\rootFunction{I^j}{j}\in \Ninv(\omega)$, this reduced expression can not use the letter $q_j$; so, it is a reduced expression of $\omega$ in $Q_{[m]\smallsetminus I^{j}}$ as desired.
    ...
    Now let $\widetilde Q$ be a subword of $Q_{[m]\smallsetminus I^{j-1}}=Q_{[m]\smallsetminus I^{j}}$ which is a reduced expression of $\omega$ with prefix $Q^{j-1}$, and let $\widetilde I$ be the corresponding facet.
    ...
    \item[(3)(a)] If $\rootFunction{I^j}{j}\in \Inv(\omega) \cap \Ninv (\pi)$ and $Q_{[m]\smallsetminus I^{j-1}\smallsetminus \{j\}}$ contains a reduced expression of $\omega$ with prefix $Q^{j-1}$, then $Q^j=Q^{j-1}$.
...
\begin{enumerate}
    \item If $\rootFunction{I^j}{j}\in \Ninv(\omega)$ then clearly $j\in I$ is forced. Otherwise $Q_{[m]\smallsetminus I}$ would not be a reduced expression of $\omega$.
    ...
    \item[(3)(a)] If $\rootFunction{I^j}{j}\in \Inv(\omega) \cap \Ninv (\pi)$ and $Q_{[m]\smallsetminus I^{j-1}\smallsetminus \{j\}}$ contains a reduced expression of $\omega$ with prefix $Q^{j-1}$ then $j\in I^j$ is forced. 
    ...
    \item[(3)(b)]  If $\rootFunction{I^j}{j}\in \Inv(\omega) \cap \Ninv (\pi)$ and $Q_{[m]\smallsetminus I^{j-1}\smallsetminus \{j\}}$ does not contain a reduced expression of $\omega$ with prefix $Q^{j-1}$ then $j\notin I^j$ is forced. Otherwise, the complement of $I$ would not be a reduced expression of $\omega$.
