...
Finally, we show that all pipe dreams of $0\omega$ are acyclic whenever $\omega$ is dominant. Therefore, Theorem~\ref{thm:A} implies that the $\nu$-Tamari lattice is a quotient of the interval~$[e,\omega]$ for a well-chosen dominant permutation $\omega$ (\cref{subsec:nuTamari}).
...
Given a pipe dream $P$, we write $i\to j$ whenever $i$ has an elbow weakly northwest of $j$. Lemma~\ref{lem:rectangle} gives us that $i\to j$ implies $i\contactLess{P} j$. 
...
Given a reduced cycle $\bf C$, we can also assume that $i_1=\min{i_1,\ldots,i_m}$.
...
For the converse, assume we have a pipe dream $P$ that is not acyclic. The contact graph must contain a reduced cycle ${\bf C}=(i, j_1, \ldots, j_m)$ with $i<j_k$ for all $1\le k\le m$. 
If $m=1$, then $i$ and $j_1$ must cross , $i\to j_1$ implies there is an elbow of $i$ southwest of the crossing and $j_1\to i$ implies there is an elbow of $j$ northeast of the crossing. Lemma~\ref{lem:not_dominant} implies $\omega$ is not dominant. We consider the case where $m>1$.
For $i\to j_1$, the pipe $i$ must have an  elbows at $(a,b)$ weakly northwest of $(c_1,d_1)$ an elbow of the pipe $j_1$. 
Similarly $j_m\to i$ must have $(a_m,b_m)$ weakly northwest $(c,d)$ elbows of the pipes $j_m$ and $i$, respectively. 
Since the cycle $C$ is reduced, we must have that $(a_m,b_m)$ is not northeast $(c_1,d_1)$. That is $c_1<a_m$ or $d_1<b_m$.

For $d_1<b_m$, we must have $a_m<c_1$ since the pipe $i$ is northwest of $(c_1,d_1)$ and southeast of $(a_m,d_m)$.
The pipe $j_1$ cannot have an elbow southeast of $(a_m,b_m)$; otherwise $j_m\to j_1$ would contradict the fact that $\bf C$ is reduced.
The pipes $i<j_1$ must cross in the some column $<a_m$ since $j_1$ must remain strictly west of $a_m$ in rows $>b_m$, but the pipe $i$
must have and elbow southeast of $(a_m,b_m)$. Now, in rows $\le b_m$ the pipe $j_1$ must remain northwest of the pipe $i$. 
It is them impossible to have an elbow in position $(c_1,d_1)$ southeast of $i$ in a row $d_1<b_1$. This is a contradiction and it exclude this case.

For $c_1<a_m$, similarly to above,  we must now have $b_m<d_1$ and the pipe $j_m$ cannot have an elbow northeast of $(c_1,d_1)$.
The pipes $i<j_m$ must cross in some column strictly between $c_1$ and $a_m$; the pipe $j_m$ must remain strictly south of $d_1$ in columns $<c_1$, but the pipe $i$
must have an elbow northwest of $(c_1,d_m)$. Then, $j_m$ has an elbow at $(a_m,b_m)$ northwest of $i$.
...
the pipe $i$ has an elbow at $(a,b)$ southeast of the crossing since $a\le c_1$ and the pipe $j_m$ has an elbow at $(a_m,b_m)$ northeast of the crossing.
 Lemma~\ref{lem:not_dominant} implies that $\omega$ is not dominant and conclude the proof.
\end{proof}
