...
The \defn{weak order} on $W$ is the partial order $\le$ defined by $\sigma \leq \omega$ if there exists $\tau \in S$  such that $\sigma\tau = \omega$ and $\ell(\sigma) + \ell(\tau) = \ell(\omega)$.
...
\end{figure}
\vincent{Do we want to redo this picture using sorting networks?}
\end{example}
