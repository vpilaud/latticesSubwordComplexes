...
The first counter-example happens for the exiting permutation~${\omega = 1243}$.
\end{remark}
...
\begin{figure}[b]
	...
	\caption{Sweeping algorithm for the permutations~$\pi = 512364$ and~$\omega = 561324$.} % See details in \cref{exm:sweepingAlgorithm}.}
...
\begin{example}
\vincent{todo: Introduce here quickly pseudoline arrangements on sorting networks and their contact graphs.}
\end{example}
...
See \cref{fig:increasingFlipPosets}\,(left) for an example with~$c = \tau_2\tau_1\tau_3$ in type~$A_3$.
See also \cref{fig:subwordComplex1}.
\end{example}
...
The increasing flip lattice of this subword complex is represented in \cref{fig:increasingFlipPosets}, and the highlighted facets in blue and red show that it is not a lattice (this was observed in~\cite[Rem.~5.12]{PilaudStump-brickPolytope}).
See also \cref{fig:subwordComplex3}.
%
\begin{figure}[t]
	\centerline{
	...
	\caption{The increasing flip graph on~$\subwordComplex(Q, \wo)$ where~$Q = \tau_2 \tau_1 \tau_3 \tau_2 \tau_1 \tau_3 \tau_2 \tau_1 \tau_3$ (left) and~$Q = \tau_1 \tau_2 \tau_3 \tau_2 \tau_1 \tau_2 \tau_3 \tau_2 \tau_1$ (right). The left one is the $\tau_2 \tau_1 \tau_3$-Cambrian lattice, while the right one is not a lattice (the two facets highlighted in blue have no join while the two facets highlighted in red have no meet). In both examples, all facets are acyclic.}
...
The first $3$ use~$\omega = \wo$ and are spherical subword complexes (their brick polytopes are represented in \cref{fig:brickPolytope1,fig:brickPolytope2,fig:brickPolytope3}), while the last $2$ do not.
%
...
{For~$Q = \tau_2 \tau_1 \tau_3 \tau_2 \tau_1 \tau_3 \tau_2 \tau_1 \tau_3$ and~$\omega = \wo$, the subword complex equivalence is a lattice congruence (left) so that the increasing flip poset is a lattice (right). It is actually the $\tau_2\tau_1\tau_3$-Cambrian lattice. The blue bubbles represent the classes of the pipe dream congruence. For each facet of the subword complex, we have represented the corresponding pseudoline arrangement on the sorting network, and the corresponding contact graph. Note that all facets are acyclic in this case.}
{fig:subwordComplex1}
%
\hvFloat[floatPos=p, capWidth=h, capPos=r, capAngle=90, objectAngle=90, capVPos=c, objectPos=c]{figure}
{\includegraphics[scale=.7]{subwordComplex2}}
{For~$Q = \tau_2 \tau_3 \tau_1 \tau_3 \tau_2 \tau_1 \tau_2 \tau_3 \tau_1$ and~$\omega = \wo$, the subword complex equivalence is not a lattice congruence (left) but the increasing flip poset is a lattice (right). The blue bubbles represent the classes of the pipe dream congruence. For each facet of the subword complex, we have represented the corresponding pseudoline arrangement on the sorting network, and the corresponding contact graph. Note that all facets are acyclic in this case.}
{fig:subwordComplex2}
%
\hvFloat[floatPos=p, capWidth=h, capPos=r, capAngle=90, objectAngle=90, capVPos=c, objectPos=c]{figure}
{\includegraphics[scale=.7]{subwordComplex3}}
{For~$Q = \tau_1 \tau_2 \tau_3 \tau_2 \tau_1 \tau_2 \tau_3 \tau_2 \tau_1$ and~$\omega = \wo$, the subword complex equivalence is not a lattice congruence (left) and the increasing flip poset is not a lattice (right). The blue bubbles represent the classes of the pipe dream congruence. For each facet of the subword complex, we have represented the corresponding pseudoline arrangement on the sorting network, and the corresponding contact graph. Note that all facets are acyclic in this case.}
{fig:subwordComplex3}
\end{example}
...
%
\begin{figure}[p]
	\centerline{\includegraphics[scale=.8]{subwordComplex4}}
	\caption{The sets~$\linearExtensions(I)$ (blue bulles) are not always intervals.}
...
%
\begin{figure}[p]
	\centerline{\includegraphics[scale=.8]{subwordComplex5}}
	\caption{The sets~$\linearExtensions(I)$ (blue bubbles) are not always subsets of, and may even not meet~$[e,\omega]$ (green bubble).}
...
\nantel{it is bad to name this ``congruence". Call it equivalence, and state it is a concruence in the conjecture.}
...
\begin{example}
\vincent{todo: explain}
\begin{figure}[p]
	\centerline{\input{figures/brickPolytope1}}
	\caption{The brick polytope of the subword complexe...}
...
\nantel{to be honest, I find that the paper ends well here, the rest should be part of a paper by No\'emie alone?}
