...
This property is known to be true when $\omega=n\dots21$ is the reverse permutation. In this case, the increasing flip poset is the classical Tamari lattice. Removing the vertex 1 from the contact graph of a pipe dream~$P\in \pipeDreams(1\oplus \omega)$ returns the binary tree corresponding to $P$ (with the leaves removed and the internal nodes labeled appropriately with the numbers from 2 to $n+1$). The edges of the binary tree are oriented going away from the root of the tree.  
