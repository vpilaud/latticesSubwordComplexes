...
\newcommand{\strictLinearExtensions}{\mathcal{L}^\star} % strict linear extensions
...
\newcommand{\subwordStrictAcyclicFacets}{\mathcal{F}^\star} % subword complex
...
A flip in a pipe dream exchanges a contact with a crossing between two pipes, and the flip is increasing when the contact is southwest of the crossing involved in the flip.
...
This can seems to be fixed by an additional assumption on~$Q$.
...
In contrast to the case of pipe dreams, the Hasse diagram of the quotient~$[e, \omega]/{\equiv_{Q, \omega}}$ is not isomorphic to the increasing flip graph on acyclic facets of~$\subwordComplex(Q, \omega)$ for two reasons:
\begin{itemize}
...
\begin{conjectureA}
If~$Q$ is alternating, then the Hasse diagram of the quotient~$[e, \omega]/\equiv_{Q, \omega}$ is isomorphic to the graph of extremal increasing flips between strongly acyclic facets of~$\subwordAcyclicFacets(Q, \omega)$.
\end{conjectureA}

Combining the sorting and alternating conditions, we thus obtain the following conjecture, which can be seen as the natural extension of our pipe dream result.

\begin{conjectureA}
If~$Q$ is sorting and alternating, then the Hasse diagram of the quotient~$[e, \omega]/\equiv_{Q, \omega}$ is isomorphic to the graph of extremal increasing flips between acyclic facets of~$\subwordAcyclicFacets(Q, \omega)$.
\end{conjectureA}

This last conjecture has a strong connection to the geometry of subword complexes given by brick polytopes of~\cite{PilaudSantos-brickPolytope, PilaudStump-brickPolytope} and the brick polyhedra of~\cite{JahnStump}.
Namely, the extremal increasing flips between acyclic facets of~$\subwordAcyclicFacets(Q, \omega)$ is precisely the bounded part (meaning forgetting the unbounded rays) of the brick polyhedron~$\brickPolyhedron(Q, \omega)$ oriented by a natural linear functional.
In particular, when~$\omega = \wo$, we obtain the following conjecture, extending results from~\cite{Pilaud-brickAlgebra}.

\begin{conjectureA}
If~$Q$ is sorting and alternating, the oriented graph of the brick polytope~$\brickPolyhedron(Q, \wo)$ is the Hasse diagram of a lattice quotient of the weak order.
\end{conjectureA}
...
For a facet~$I$ of~$\subwordComplex(Q, \omega)$, we define~$\strictLinearExtensions(I) \eqdef \linearExtensions(I) \cap [e, \omega]$.
We say that~$I$ is \defn{strictly acyclic}~$\strictLinearExtensions(I) \ne \varnothing$, and we denote by~$\subwordStrictAcyclicFacets(Q, \omega)$ the set of strictly acyclic facets of~$\subwordComplex(Q, \omega)$.
Note that by \cref{thm:linearExtensionsPartitionSubwordComplexA}~\eqref{item:partition}, we have
\[
[e, \omega] = \bigsqcup_{I\in\subwordStrictAcyclicFacets(Q,\omega)} \strictLinearExtensions(I).
\]
...
For a non-empty subword complex $\subwordComplex(Q,\omega)$, the \defn{subword complex congruence} is the equivalence relation~$\equiv_{Q,\omega}$ on the interval~$[e, \omega]$  whose equivalence classes are the sets~$\strictLinearExtensions(I)$ of linear extensions of strictly acyclic facets~$I$ of~$\subwordStrictAcyclicFacets(Q, \omega)$.
...
First, its elements correspond to the congruence classes of~$\equiv_{Q, \omega}$, hence to the strictly acyclic facets in~$\subwordStrictAcyclicFacets(Q, \omega)$.
The cover relations are certain increasing flips between the facets in~$\subwordStrictAcyclicFacets(Q, \omega)$.
However, in contrast to \cref{thm:pipeDreamQuotient}, not all increasing flips between two facets in~$\subwordStrictAcyclicFacets(Q, \omega)$ yields a cover relation of~$\subwordStrictAcyclicFacets(Q, \omega)$, as illustrated by the following example.
...
We say that a flip between two facets~$I, J \in \subwordStrictAcyclicFacets(Q, \omega)$ with~$I \ssm \{i\} = J \ssm \{j\}$ is \defn{extremal} if the root~$\rootFunction{I}{i}$ is a ray of the root configuration~$\Roots(I)$ (or equivalently, $\rootFunction{J}{j}$ is a ray~$\Roots(J)$).
...
For a non-empty subword complex $\subwordComplex(Q,\omega)$ where~$Q$ is alternating, the Hasse diagram of the lattice quotient~$[e, \omega]/\equiv_{Q, \omega}$ is isomorphic to the graph of extremal increasing flips between strictly acyclic facets of~$\subwordStrictAcyclicFacets(Q, \omega)$.
\end{conjecture}

\cref{conj:latticeCongruenceAlternating,conj:latticeQuotientAlternating} holds in type~$A_n$: our specific proof of~\cref{thm:pipeDreamCongruence,thm:pipeDreamQuotient} can be extended to arbitrary alternating words in type~$A_n$ as will be shown in~\cite{Cartier}.
They are also supported by computer experiments: we verified it for all alternating words of length at most~$\ell(\wo)$ in types~$B_2, B_3, D_4$ and~$H_3$.
\vincent{Noemie check that.} 
...
In this case, all acyclic facets are strictly acyclic by \cref{thm:linearExtensionsPartitionSubwordComplexB}.
...
For a non-empty subword complex $\subwordComplex(Q,\omega)$ where~$Q$ is sorting and alternating, the Hasse diagram of the lattice quotient~$[e, \omega]/\equiv_{Q, \omega}$ is isomorphic to the graph of extremal increasing flips between acyclic facets of~$\subwordAcyclicFacets(Q, \omega)$.
\end{conjecture}
...
\begin{conjecture}
\label{conj:latticeQuotientSortingAlternatingGeometric}
For a non-empty subword complex $\subwordComplex(Q,\omega)$ where~$Q$ is sorting and alternating, the finite part of the oriented graph of the brick polyhedron~$\brickPolyhedron(Q, \omega)$ is isomorphic to the Hasse diagram of the lattice quotient~$[e, \omega]/\equiv_{Q, \omega}$.
\end{conjecture}
...
But this is not the case as shown in \cref{exm:acyclicFlipvsWeakOrder}.     
\end{remark}

\begin{example}\label{exm:acyclicFlipvsWeakOrder}
    Let $W=S_3$ be the symmetric group generated by simple transpositions $S=\{s_1,s_2\}$ with $s_i=(i, i+1 )$.
Let $Q=(s_1,s_2,s_1,s_2,s_1,s_2)$ and $\omega=\wo=s_1s_2s_1=s_2s_1s_2$.
The subword complex $\subwordComplex(Q,\omega)$ has eight facets, six of which are acyclic.
    All the fibers of $\equiv_{Q,\omega}$ are singletons and the lattice quotient~$[e,\omega]/\equiv_{Q,\omega}$ coincides with the weak order of type $A_2$.
