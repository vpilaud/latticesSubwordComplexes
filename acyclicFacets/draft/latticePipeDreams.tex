...
For example, \cref{fig:pipeDreams} illustrates an increasing from the left pipe dream to the right pipe dream.
%
...
As observed in~\cite{Woo, PilaudPocchiola, Pilaud-these, Stump}, the family of reversing pipe dreams belong to the Catalan families.
...
We now gather some elementary properties of crossings and contacts that will be needed later to construct pipe dreams from permutations.
...
In this section, we describe three algorithms to decide whether two permutations~$\pi$ and~$\pi'$ are equivalent for the pipe dream congruence~$\equiv_\omega$.
The first two algorithms actually construct the unique pipe dream~$P \in \acyclicPipeDreams(\omega)$ of which~$\pi$ is a linear extension, while the last algorithm uses a rewriting technique.
...
Our first algorithm is a \defn{sweeping algorithm} that constructs the unique pipe dream~$P \in \acyclicPipeDreams(\omega)$ of which~$\pi$ is a linear extension.
It is inspired by the algorithm to compute greedy pipe dreams~\cite{PilaudPocchiola, PilaudStump-ELlabelings}.
...
For any permutations~$\pi,\omega \in \fS_n$ such that~$\pi < \omega$ in weak order, the unique pipe dream~$P \in \acyclicPipeDreams(\omega)$ such that~$\pi \in \linearExtensions(P)$ can be constructed by sweeping the triangular shape in any perturbation of the northwest direction and inserting a crossing when sweeping a vertex~$v$ of the grid where pipe~$i$ arrives horizontally and pipe~$j$ arrives vertically if and only if
\begin{itemize}
...
More precisely, at step~$t$, we consider the set~$X_t$ of free southeast elbows in the rectangle~$[\pi(t)] \times [\omega^{-1}(\pi(t))]$, and we insert a pipe starting at row~$\pi(t)$, ending at column~$\omega^{-1}(\pi(t))$, covering each elbow of~$X_t$, and adding a new free southeast elbow in between any two consecutive elbows of~$X_t$
...
\vincent{I am still working below this line.}
...
\begin{proof}
Fix~$t \in [n]$.
At the beginning of the algorithm, there is no southeast contact in the rectangle~$R_t \eqdef [\pi(t)] \times [\omega^{-1}(\pi(t))]$.
...
\begin{itemize}
\item If~$\pi(s) < \pi(t)$ and~$\omega^{-1}\big( \pi(s) \big) < \omega^{-1}\big( \pi(t) \big)$, then the pipe~$\pi(s)$ is completely contained in the rectangle~$R_t$. Since the pipe~$\pi(s)$ has one more northwest contact than southeast contact, its insertion creates a new free northwest contact in the rectangle~$R_t$.
\item If~$\pi(s) < \pi(t)$ and~$\omega^{-1}\big( \pi(s) \big) > \omega^{-1}\big( \pi(t) \big)$, then the pipe~$\pi(s)$ enters and exits the rectangle~$R_t$ with an horizontal step, so that it has as many northwest and southeast contacts in~$R_t$, and its insertion does not modify the number of free northwest contacts in the rectangle~$R_t$.
\item If~$\pi(s) > \pi(t)$ and~$\omega^{-1}\big( \pi(s) \big) < \omega^{-1}\big( \pi(t) \big)$, then the pipe~$\pi(s)$ enters and exits the rectangle~$R_t$ with a vertical step, so that it has as many northwest and southeast contacts in~$R_t$, and its insertion does not modify the number of free northwest contacts in the rectangle~$R_t$.
...
\begin{proof}
The algorithm constructs a collection~$P$ of $n$ pairwise disjoint pipes except at crossings and contacts.
...
\]
many $\se$ contacts by \cref{lem:rectangleInsertionAlgorithm} and since~$\pi < \omega$ in weak order.
...
\begin{proof}
By construction of the insertion algorithm, all southeast contacts of the pipe~$\pi(t)$ inserted at step~$t$ are in contact with northwest contacts of pipes~$\pi(s)$ inserted at steps~$s < t$.
...
\vincent{Explain with the example of the sylvester congruence}
...
\vincent{I want to think about congruences of the interval $[e,\omega]$ in general.}
