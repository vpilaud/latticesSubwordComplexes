...
Although the algorithm presented in~\cite{JahnStump} is more general, our sweeping algorithm is more direct when computing the output for the input $\pi$, since we skip the step of translating the conditions on the sign of $f(\rootFunction{I^j}{j})$ in Jahn-Stump's algorithm. We remark that these two algorithms were developed independently, while approaching different problems. This highlights the relevance of the sweeping algorithm.  
\end{remark}
