...
Note that throughout the proof, the pipes, rows, and column are all indexed by indices ranging from $0$ to~$n$.
...
\begin{enumerate}[(i)]
\item Assume first that~$k$ and~$\ell$ cross at~$x$. Since $\omega^{-1}(k) < \omega^{-1}(\ell)$, we have~$k \contactLess{P} \ell$. Moreover, since~$\ell\ne i$, the pipe~$\ell$ must have an elbow west of~$x$ along row~$i$, which implies that~$\ell \contactLess{P} k$. Hence, we have both~$k \contactLess{P} \ell$ and~$\ell \contactLess{P} k$, so that~$P$ is cyclic.
\item Assume now that~$k$ and~$\ell$ touch at~$x$, so that~$k \contactLess{P} \ell$. If~$\ell < k$, then~$k$ and~$\ell$ must cross before~$x$, and flipping~$x$ in~$P$ yields a cyclic facet. If~$\omega^{-1}(\ell) < \omega^{-1}(k)$, then~$\ell \contactLess{P} k$, so that~$P$ is again cyclic. Hence, we can assume that~$k < \ell$ and~$\omega^{-1}(k) < \omega^{-1}(\ell)$. By maximality of~$k$, we obtain that~$j$ and~$\ell$ do not cross, and thus that~$j$ passes northwest of~$x$. We conclude that~$j \contactLess{P} k$. Since~$\omega^{-1}(k) < \omega^{-1}(j)$, we also have~$k \contactLess{P} j$, so that~$P$ is cyclic.
\end{enumerate}
