...
Our first algorithm to construct the unique pipe dream~$P \in \acyclicPipeDreams(\omega)$ with~$\pi \in \linearExtensions(P)$ is a \defn{sweeping algorithm}, inspired by the algorithm to compute greedy pipe dreams~\cite{PilaudPocchiola, PilaudStump-ELlabelings}.
...
%\begin{corollary}
%For any~$\pi \le \omega$, the insertion algorithm creates a pipe dream of~$\pipeDreams(\omega)$.
%\end{corollary}
...
Recall from \cref{exm:Tamari3} that the sylvester congruence can be defined as the transitive closure of the classical rewriting rule~$UikVjW \equiv UkiVjW$ for~$i < j < k$ (as usual, we write the permutations of~$\fS_n$ as words in one-line notation).
We now describe a similar rewriting rule for the pipe dream congruence~$\equiv_\omega$.

\begin{proposition}
\label{prop:rewritingRule}
On the interval~$[e,\omega]$ of the weak order, the pipe dream congruence~$\equiv_\omega$ coincides with the transitive closure of the rewriting rule~$U ik V \equiv_\omega U ki V$ where~$i < k$ are elements of~$[n]$ while~$U, V$ are (possibly empty) words on~$[n]$ such that
\[
\#\set{j \in U}{i < j < k \text{ and } \omega^{-1}(i) > \omega^{-1}(j) > \omega^{-1}(k)} \ge \#\set{j \in U}{j < i \text{ and } \omega^{-1}(j) < \omega^{-1}(k)}.
%\#\set{j \in U}{i < j < k \text{ and } \omega^{-1}(i) > \omega^{-1}(j) > \omega^{-1}(k)} \ge \#\set{j < i}{\omega^{-1}(j) < \omega^{-1}(k)},
\]
\vincent{Note that $b \in U$ on the right hand side by \cref{lem:interval}.}
% The letters~$j \in U$ such that~$i < j < k$ while~$\omega^{-1}(i) > \omega^{-1}(j) > \omega^{-1}(k)$ are called \defn{pipe dream congruence witnesses} for the exchange of~$i$ and~$k$.
